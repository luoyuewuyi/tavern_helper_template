...
description: Build or update Tavern character-card projects inside this repository. Use when Codex needs to create or modify 酒馆角色卡、世界书、脚本、状态栏、前端界面、变量界面、MVU 界面、正则替换界面, especially when the user gives a project name and expects Codex to stage or pull the project, work under src, run pnpm build when implementation changes require it, and stop at a locally prepared result for manual push or import.
---
